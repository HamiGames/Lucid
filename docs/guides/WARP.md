...
1. **Encryption**: XChaCha20-Poly1305 per-chunk with session-derived keys

1. **Anchoring**: Merkle root (BLAKE3) → On-System Data Chain (LucidAnchors)

1. **Payouts**: Monthly USDT-TRC20 via PayoutRouterV0/PRKYC on TRON
...
1. **Admin GUI**: Pi provisioning, manifests, payouts, key rotation

1. **Node GUI**: PoOT metrics, wallet management, payout batches
...
1. **ASSUMPTIONS**: Must be "None" use build referrence materials (./Build_guide_docs/ *.txt and HamiGames/Lucid)

1. **PLAN/COMMANDS**: Numbered, minimal, copy-paste ready

1. **ACCEPTANCE**: Explicit checks/tests to prove success
