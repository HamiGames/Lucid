...
This script serves as the entrypoint for the session-processor container,
...
if __name__ == "__main__":
    ...
    port_str = os.getenv('SESSION_PROCESSOR_PORT', '8091')
    host = '0.0.0.0'  # Always bind to all interfaces in container
    
    try:
        port = int(port_str)
    except ValueError:
        print(f"ERROR: Invalid SESSION_PROCESSOR_PORT value: {port_str}", file=sys.stderr)
        sys.exit(1)
    ...
    uvicorn.run('sessions.processor.main:app', host=host, port=port)
