#!/bin/bash
# ========================================================================
# File: /app/gui_api_bridge/scripts/gui-exec.sh
# x-lucid-file-path: /app/gui_api_bridge/scripts/gui-exec.sh
...
execute_in_docker() {
    ...
    docker exec -i lucid-gui_api_bridge bash -c "$cmd"
}
...
docker_logs() {
    ...
    docker logs -f lucid-gui_api_bridge
}

docker_stats() {
    """Show Docker container stats"""
    log_info "GUI API Bridge container stats:"
    docker stats lucid-gui_api_bridge --no-stream
}
