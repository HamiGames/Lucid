#!/bin/bash
# ========================================================================
# File: /app/gui_api_bridge/scripts/os-detector-linux-enforcer.sh
# x-lucid-file-path: /app/gui_api_bridge/scripts/os-detector-linux-enforcer.sh
...
ensure_gui_api_bridge_running() {
    ...
    case "$runtime" in
        docker)
            if docker ps | grep -q "lucid-gui_api_bridge"; then
            ...
            cd "$WORKSPACE_DIR"
            docker-compose -f gui_api_bridge/docker-compose.yml up -d lucid-gui_api_bridge
        ...
        pi-native|ssh)
            ...
            if check_ssh_available; then
                if ssh -o BatchMode=yes "pickme@${PI_HOST:-192.168.0.75}" "docker ps | grep -q lucid-gui_api_bridge"; then
