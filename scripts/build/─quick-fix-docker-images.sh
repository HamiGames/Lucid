...
build_auth_service() {
    ...
    if docker buildx build \
        --platform "$PLATFORM" \
        -t "$REGISTRY/lucid-auth-service:$BUILD_TAG" \
        -f Dockerfile \
        --push \
        .; then
...
build_storage_database() {
    ...
    if docker buildx build \
        --platform "$PLATFORM" \
        -t "$REGISTRY/lucid-storage-database:$BUILD_TAG" \
        -f Dockerfile \
        --push \
        .; then
...
build_mongodb() {
    ...
    if docker buildx build \
        --platform "$PLATFORM" \
        -t "$REGISTRY/lucid-mongodb:$BUILD_TAG" \
...
build_redis() {
    ...
    if docker buildx build \
        --platform "$PLATFORM" \
        -t "$REGISTRY/lucid-redis:$BUILD_TAG" \
        -f Dockerfile.redis \
        --push \
        .; then
...
build_elasticsearch() {
    ...
    if docker buildx build \
        --platform "$PLATFORM" \
        -t "$REGISTRY/lucid-elasticsearch:$BUILD_TAG" \
