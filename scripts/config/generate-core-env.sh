...
# File: /app/scripts/config/generate-core-env.sh
# x-lucid-file-path: /app/scripts/config/generate-core-env.sh
...
validate_pi_mounts() {
    ...
    if [[ -d /mnt/myssd && ! -d /mnt/myssd/Lucid ]]; then
        echo "WARNING: /mnt/myssd present but Lucid tree not found; using $PROJECT_ROOT"
    fi
}
