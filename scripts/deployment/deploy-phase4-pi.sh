...
verify_compose_file() {
    ...
    local pi_compose_file="$PI_DEPLOY_DIR/$LUCID_HOST_REL_COMPOSE_SUPPORT"
...
deploy_phase4_services() {
    ...
    ssh -o ConnectTimeout=300 -o ServerAliveInterval=60 -o ServerAliveCountMax=5 -o StrictHostKeyChecking=no -i "$PI_SSH_KEY_PATH" "$PI_USER@$PI_HOST" "
        cd $PI_DEPLOY_DIR
        docker-compose --env-file configs/environment/.env.foundation --env-file configs/environment/.env.core --env-file configs/environment/.env.application --env-file configs/environment/.env.support -f "${LUCID_HOST_COMPOSE_FOUNDATION}" -f "${LUCID_HOST_COMPOSE_CORE}" -f "${LUCID_HOST_COMPOSE_APPLICATION}" -f "${LUCID_HOST_COMPOSE_SUPPORT}" up -d
    " >/dev/null 2>&1
