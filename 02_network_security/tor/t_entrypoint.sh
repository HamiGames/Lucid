#!/usr/bin/env bash
# tor_entrypoint.sh — Tor Proxy Container Entrypoint v5.3.0
# File: /app/02_network_security/tor/t_entrypoint.sh
# x-lucid-file-path: /app/02_network_security/tor/t_entrypoint.sh
...
#   /run/lucid/tor       — container runtime data dir  (Tor data, keys, state)
...
# ALL paths point into /run/lucid/tor (container data dir),
# /tmp/lucid/tor (container ephemeral scratch), or /opt/lucid/tor (container
# static assets).  /var/lib/tor, /var/log/tor, /run/tor are Debian host paths
# and do NOT exist in this container.
# ─────────────────────────────────────────────────────────────────────────────
TOR_DATA_DIR="${TOR_DATA_DIR:-/run/lucid/tor}"
TOR_CONFIG_DIR="${TOR_CONFIG_DIR:-/opt/etc/tor}"
...
hex_encode_file() {
    ...
    if result="$(busybox od -An -tx1 "$file" 2>/dev/null \
       ...
       && [[ -n "$result" ]]; then
        ...
        # Pure bash fallback — binary-safe, no external tools required.
        # LC_ALL=C disables multibyte so read -n 1 gets exactly one raw byte.
        # printf '%d' "'${char}" is POSIX bash character-ordinal notation (0-255).
        # CAVEAT: bash read strips trailing NUL bytes — final cookie hex may be
        # shorter than COOKIE_HEX_LENGTH if the file ends with NUL bytes.
...
# =============================================================================
# Migrate legacy data (no-op if /run/lucid/tor is already populated)
# =============================================================================
...
migrate_legacy_var_lib_tor() {
    ...
    for item in "${legacy_dir}"/lucid_*; do
        [[ -e "${item}" ]] || continue

        local base
        base="$(basename "${item}")"
        local dst="${TOR_DATA_DIR}/${base}"

        # Skip build-time marker files
        if [[ "${item}" == *.keep ]]; then
            log_debug "migrate_legacy: skip marker ${item}"
            continue
        fi

        if [[ -d "${item}" ]]; then
            ensure_dir "${dst}" "${TOR_USER}"
            if cp -a "${item}/." "${dst}/"; then
                chown -R "${TOR_USER}:${TOR_GROUP}" "${dst}" 2>/dev/null || true
                log "migrate_legacy: ${item} → ${dst}"
                (( ++migrated )) || true
            else
                log_warn "migrate_legacy: cp -a failed for ${item}"
            fi
        elif [[ -f "${item}" ]]; then
            if cp -p "${item}" "${dst}"; then
                chown "${TOR_USER}:${TOR_GROUP}" "${dst}" 2>/dev/null || true
                log "migrate_legacy: ${item} → ${dst}"
                (( ++migrated )) || true
            else
                log_warn "migrate_legacy: cp failed for ${item}"
            fi
        fi
    done

    if (( migrated == 0 )); then
        log_debug "migrate_legacy: nothing to migrate"
    else
        log "migrate_legacy: ${migrated} item(s) moved to ${TOR_DATA_DIR}"
    fi
}
...
# /tmp/lucid/tor/control_auth_cookie (the persistent failsafe source).
...
# /tmp/lucid/tor/control_auth_cookie persists until container stop.
generate_control_auth_cookie_preinit() {
    ...
    # Ensure /tmp/lucid/tor/ directory structure exists (container scratch dir).
    if [[ ! -d /tmp/lucid/tor ]]; then
        if mkdir -p /tmp/lucid/tor; then
            log_debug "Created /tmp/lucid/tor/"
        else
            log_error "Cannot create /tmp/lucid/tor/ — fatal"
            exit 1
        fi
    fi
    ...
    if [[ -n "${valid_source}" ]]; then
        ...
        log "Preinit complete — ${TOR_COOKIE_TMP} is persistent failsafe source"
    fi
...
# Writes /tmp/lucid/tor/control_auth_cookie backup as fail-safe source for
...
wait_for_cookie() {
    ...
    # Write /tmp/lucid/tor backup — fail-safe source for copy_cookie_to_shared.
    local tmp_dir
    ...
    if cp "${TOR_COOKIE_FILE}" "${TOR_COOKIE_TMP}" 2>/dev/null; then
        ...
        log_warn "Could not write cookie backup — continuing without it"
    fi
...
copy_cookie_to_shared() {
    ...
    if (( cookie_sz != COOKIE_SIZE )); then
        ...
        # Container-internal search paths only.
...
# =============================================================================
_cleanup() {
    trap - EXIT   # Prevent recursive invocation if exit fires during cleanup
    local rc=$?
