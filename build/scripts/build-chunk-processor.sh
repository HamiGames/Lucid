#!/bin/bash
# Build script for Lucid Chunk Processor Docker image
# Target: pickme/lucid-chunk-processor:latest-arm64
# Port: 8085
...
# Configuration
IMAGE_NAME="pickme/lucid-chunk-processor"
TAG="latest-arm64"
DOCKERFILE="sessions/Dockerfile.processor"
BUILD_CONTEXT="."
SERVICE_PORT="8085"

print_status "Building Lucid Chunk Processor Docker image..."
...
if [[ ! -d "sessions/processor" ]]; then
    print_error "Source directory not found: sessions/processor"
    exit 1
fi
...
if docker build \
    ...
    print_success "Chunk Processor service ready on port $SERVICE_PORT"
    print_success "Build script completed: build-chunk-processor.sh"
else
