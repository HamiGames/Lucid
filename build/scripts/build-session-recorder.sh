#!/bin/bash
# Build script for Lucid Session Recorder Docker image
# Target: pickme/lucid-session-recorder:latest-arm64
# Port: 8084
...
# Configuration
IMAGE_NAME="pickme/lucid-session-recorder"
TAG="latest-arm64"
DOCKERFILE="sessions/Dockerfile.recorder"
BUILD_CONTEXT="."
SERVICE_PORT="8084"

print_status "Building Lucid Session Recorder Docker image..."
...
if [[ ! -d "sessions/recorder" ]]; then
    print_error "Source directory not found: sessions/recorder"
    exit 1
fi
...
if docker build \
    ...
    print_success "Session Recorder service ready on port $SERVICE_PORT"
    print_success "Build script completed: build-session-recorder.sh"
else
