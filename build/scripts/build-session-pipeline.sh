#!/bin/bash
# Build script for Lucid Session Pipeline Docker image
# Target: pickme/lucid-session-pipeline:latest-arm64
# Port: 8083
...
# Configuration
IMAGE_NAME="pickme/lucid-session-pipeline"
TAG="latest-arm64"
DOCKERFILE="sessions/Dockerfile.pipeline"
BUILD_CONTEXT="."
SERVICE_PORT="8083"

print_status "Building Lucid Session Pipeline Docker image..."
...
if [[ ! -d "sessions/pipeline" ]]; then
    print_error "Source directory not found: sessions/pipeline"
    exit 1
fi
...
if docker build \
    ...
    print_success "Session Pipeline service ready on port $SERVICE_PORT"
    print_success "Build script completed: build-session-pipeline.sh"
else
