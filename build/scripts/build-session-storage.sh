#!/bin/bash
# Build script for Lucid Session Storage Docker image
# Target: pickme/lucid-session-storage:latest-arm64
# Port: 8086
...
# Configuration
IMAGE_NAME="pickme/lucid-session-storage"
TAG="latest-arm64"
DOCKERFILE="sessions/Dockerfile.storage"
BUILD_CONTEXT="."
SERVICE_PORT="8086"

print_status "Building Lucid Session Storage Docker image..."
...
if [[ ! -d "sessions/storage" ]]; then
    print_error "Source directory not found: sessions/storage"
    exit 1
fi
...
if docker build \
    ...
    print_success "Session Storage service ready on port $SERVICE_PORT"
    print_success "Build script completed: build-session-storage.sh"
else
