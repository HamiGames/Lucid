...
# We want /app in sys.path, not /app/node
...
if __name__ == "__main__":
    ...
    try:
        # Verify node is a package before importing
        node_path = os.path.join(app_path, 'node')
        init_file = os.path.join(node_path, '__init__.py')
        if not os.path.exists(init_file):
            print(f"ERROR: node/__init__.py not found at {init_file}", file=sys.stderr)
            print(f"ERROR: node directory exists: {os.path.exists(node_path)}", file=sys.stderr)
            if os.path.exists(node_path):
                try:
                    contents = os.listdir(node_path)
                    print(f"ERROR: node directory contents: {contents}", file=sys.stderr)
                except Exception as list_err:
                    print(f"ERROR: Could not list node directory: {list_err}", file=sys.stderr)
            sys.exit(1)
        
        from node.main import app
    except ImportError as e:
        print(f"ERROR: Failed to import node.main: {e}", file=sys.stderr)
        print(f"ERROR: Python path: {sys.path}", file=sys.stderr)
