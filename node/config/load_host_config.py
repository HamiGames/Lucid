...
ENV_NODE_SERVICE_DIR = "LUCID_NODE_SERVICE_CONFIG_DIR"
ENV_NODE_ENDPOINTS_FILE = "LUCID_NODE_ENDPOINTS_FILE"
DEFAULT_SERVICE_DIR = Path("/app/service_configs")
DEFAULT_ENDPOINTS_FILE = "node-endpoints.yml"


def _node_package_dir() -> Path:
    return Path(__file__).resolve().parent


def resolve_node_service_config_dir() -> Path:
    env = os.environ.get(ENV_NODE_SERVICE_DIR, "").strip()
    if env:
        return Path(env)
    for cand in (
        DEFAULT_SERVICE_DIR,
        ...
        if cand.is_dir():
            return cand
    return _node_package_dir()


def resolve_node_endpoints_path() -> Optional[Path]:
    name = os.environ.get(ENV_NODE_ENDPOINTS_FILE, "").strip() or DEFAULT_ENDPOINTS_FILE
    return resolve_service_config_path(
        resolve_node_service_config_dir(),
        name,
        fallbacks=(_node_package_dir() / name,),
    )


def load_node_host_context(
    host_config_path: Optional[Path | str] = None,
) -> Tuple[Mapping[str, Any], Dict[str, ServiceEndpoint], Mapping[str, Any]]:
    """Host registry + merged node-endpoints (if present)."""
    raw, registry = load_host_registry(host_config_path or default_host_config_path())
    endpoints_path = resolve_node_endpoints_path()
    endpoints = load_yaml_file(endpoints_path) if endpoints_path else {}
    return raw, registry, endpoints


def load_node_merged_config(
    host_config_path: Optional[Path | str] = None,
) -> Dict[str, Any]:
    """Single dict: host metadata + services snapshot + node endpoints (flexible consumers)."""
    raw, registry, node = load_node_host_context(host_config_path)
    ...
    return merge_config_layers(
        ...
        {"services": snap, "node_endpoints": node},
    )


def node_gateway_endpoint(registry: Dict[str, ServiceEndpoint]) -> Optional[ServiceEndpoint]:
    """Prefer api-gateway service_name from host-config (main_lucid_gateway)."""
    return endpoint_by_service_name(registry, "node-management")
