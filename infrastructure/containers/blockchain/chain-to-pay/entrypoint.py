...
Route env: CHAIN_GATEWAY_UPSTREAM_<NAME>_HOST|PORT|TLS (NAME from route name, uppercased, non-alnum → _).
Optional per route: forward_prefix — when strip_prefix is true, upstream path = forward_prefix + remainder.
"""
...
def build_ssl_context(route: Dict[str, Any], openssl_api: Dict[str, Any]) -> ssl.SSLContext:
    ...
    cafile = os.getenv("CHAIN_TO_PAY_SSL_CA_FILE") or merged.get("cafile") or None
    certfile = os.getenv("CHAIN_TO_PAY_SSL_CERT_FILE") or merged.get("certfile") or None
    keyfile = os.getenv("CHAIN_TO_PAY_SSL_KEY_FILE") or merged.get("keyfile") or None
    if cafile == "":
        cafile = None
    if certfile == "":
        certfile = None
...
def merge_routes(doc: Dict[str, Any], env: os._Environ) -> Tuple[str, int, List[Dict[str, Any]]]:
    bind = str(doc.get("listen_bind") or env.get("CHAIN_TO_PAY_BIND", "0.0.0.0"))
    port = int(doc.get("listen_port") or env.get("CHAIN_TO_PAY_LISTEN_PORT", "8114"))
    raw = doc.get("routes")
    ...
    for r in raw:
        ...
        eh = env.get(f"CHAIN_GATEWAY_UPSTREAM_{key}_HOST")
        ep = env.get(f"CHAIN_GATEWAY_UPSTREAM_{key}_PORT")
        if eh:
            host = eh.strip()
        if ep:
            port_u = int(ep.strip())
        ...
        tls_env = os.getenv(f"CHAIN_GATEWAY_UPSTREAM_{key}_TLS")
        if tls_env is not None:
            tls = tls_env.strip().lower() in ("1", "true", "yes", "on")
        tls_profile = r.get("tls_profile")
        fp = r.get("forward_prefix")
        forward_prefix = str(fp).strip() if fp is not None else ""
        out.append(
            {
                "name": name,
...
def forward_path(path: str, route: Dict[str, Any]) -> str:
    ...
    forward_base = str(route.get("forward_prefix") or "").strip()
...
def main() -> None:
    ...
    cfg_path = Path(
        env.get(
            "CHAIN_TO_PAY_CONFIG",
            "/app/chain/config/chain-to-pay.connections.json",
        )
    )
    yaml_path = Path(
        env.get(
            "CHAIN_TO_PAY_OPENSSL_API_YAML",
            "/app/chain/config/openssl-api.yml",
        )
    )
